...
RegisterNetEvent('esx_holdupalus:currentlyRobbing')
AddEventHandler('esx_holdupalus:currentlyRobbing', function(currentStore)
	holdingUp, store = true, currentStore
end)

RegisterNetEvent('esx_holdupalus:killBlip')
AddEventHandler('esx_holdupalus:killBlip', function()
	RemoveBlip(blipRobbery)
end)

RegisterNetEvent('esx_holdupalus:setBlip')
AddEventHandler('esx_holdupalus:setBlip', function(position)
...
RegisterNetEvent('esx_holdupalus:tooFar')
AddEventHandler('esx_holdupalus:tooFar', function()
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_cancelled'))
end)

RegisterNetEvent('esx_holdupalus:robberyComplete')
AddEventHandler('esx_holdupalus:robberyComplete', function(award)
	holdingUp, store = false, ''
	ESX.ShowNotification(_U('robbery_complete', award))
end)

RegisterNetEvent('esx_holdupalus:startTimer')
AddEventHandler('esx_holdupalus:startTimer', function()
	...
	while true do
					...
					if distance < 0.5 then
						...
						if IsControlJustReleased(0, 38) then
							if IsPedArmed(PlayerPedId(), 4) then
								TriggerServerEvent('esx_holdupalus:robberyStarted', k)
							else
		...
		if holdingUp then
			...
			if Vdist(playerPos.x, playerPos.y, playerPos.z, storePos.x, storePos.y, storePos.z) > Config.MaxDistance then
				TriggerServerEvent('esx_holdupalus:tooFar', store)
			end
		end
	end
end)
